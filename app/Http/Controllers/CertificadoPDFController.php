...
class CertificadoPDFController extends Controller
{
    ...
    public function crearPDF(Request $request){
        ...
        //Renglon 1
        $consulta=DB::table('jefes')->where('clave_area','=','100000')->first();
        $rfc=$consulta->rfc;
        $personal=DB::table('personal')->where('rfc',$rfc)->first();
        ...
        if($alumno->periodos_revalidacion > 0){
            ...
            $anio_original=substr($periodo_original,0,4);
            $periodo_original_in=substr($periodo_original,4,1);
            if($periodo_original_in == 1)
                $mes="ENERO";
            else
                $mes="AGOSTO";
        }
        ...
        $texto_encapsulado.=" INTEGRAN EL PLAN DE ESTUDIOS DE ".strtoupper($carrera->nreal)." DE ";
        ...
        $qry_materias = $this->historial($control);
        ...
        if($sumacreditos >  $carrera->creditos_totales){
            $sumacreditos=$carrera->creditos_totales;
        }
        $texto_final  = "SE EXPIDE EL PRESENTE CERTIFICADO QUE AMPARA ".$sumacreditos." CREDITOS DE UN TOTAL DE ".$carrera->creditos_totales;
        $texto_final .= " QUE INTEGRAN EL PLAN DE ESTUDIOS CON CLAVE ".trim($carrera->clave_oficial)." EN LA CIUDAD DE ENSENADA, BAJA CALIFORNIA A";
        ...
        $leyenda .= " ";
        $leyenda .= ($sumacreditos<$carrera->creditos_totales)?"INCOMPLETO":"";
