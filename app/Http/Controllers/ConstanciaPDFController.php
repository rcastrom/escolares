...
//if (strlen($num) > 14) die("El número introducido es demasiado grande");
    ...
    public function crearPDF(Request $request){
        ...
        $folios=DB::table('folios_constancias')->where('anio',date('Y'))
            ->max('folio');
        $folio=$folios+1;
        DB::table('folios_constancias')->insert([
            'folio'=>$folio,
            'periodo'=>$periodo,
            'control'=>$control,
            'tipo'=>$tipo,
        ...
        $total=$this->totales($control);
        if(!empty($total)){
            $creditos_aprobados=$total[0]->creditos_aprobados;
            $clave_oficial=$total[0]->clave_oficial;
            ...
            $info=DB::table('carreras')->where('carrera',$alumno->carrera)
                ->where('reticula',$alumno->reticula)->select('clave_oficial')->first();
            $clave_oficial=$info->clave_oficial;
        }
        $ncarrera=DB::table('carreras')->where('carrera',$alumno->carrera)
            ->where('reticula',$alumno->reticula)->select('nombre_carrera','creditos_totales')->first();
        $avance = (!$creditos_aprobados)?0:round((($creditos_aprobados / $ncarrera->creditos_totales) * 100),2);
        if ($avance>100){$avance=100;}
        if((!empty($alumno->ultimo_periodo_inscrito))||(!is_null($alumno->ultimo_periodo_inscrito))||($alumno->ultimo_periodo_inscrito!="null")){
...
CRO - Curso Repetición Ordinario  CRR - Curso Repetición Regularización  EE - Exámen Especial  EA - Exámen Autodidacta CA - Estudios Convalidados EQ- Equivalencia\n\n");
            ...
            if($tipo == 'K'){
                $calificaciones = DB::select("select * from pac_constancias_kardex ('$control')");
            }else{
                $calificaciones = DB::select("select * from pac_constancias('$control')");
            }
        }
        if($tipo == 'B'){
            ...
            //$per_inicial=substr($periodo_inicial,4,1);
            ...
            $calificaciones=DB::table('historia_alumno')->where('no_de_control',$control)
        ...
        switch ($tipo){
            ...
            case 'M': {
                ...
                $mats=DB::table('seleccion_materias')->where('periodo',$periodo)
                    ...
                    ->get();
                foreach ($mats as $mat){
                    $lunes=DB::table('horarios')->where('periodo',$periodo)
                    ...
                    $martes=DB::table('horarios')->where('periodo',$periodo)
                        ->where('materia',$mat->materia)
                    ...
                    $miercoles=DB::table('horarios')->where('periodo',$periodo)
                        ->where('materia',$mat->materia)
                    ...
                    $jueves=DB::table('horarios')->where('periodo',$periodo)
                        ->where('materia',$mat->materia)
                    ...
                    $viernes=DB::table('horarios')->where('periodo',$periodo)
                        ->where('materia',$mat->materia)
                    ...
                    $sabado=DB::table('horarios')->where('periodo',$periodo)
                        ->where('materia',$mat->materia)
                    ...
                    $rfc=DB::table('horarios')->where('periodo',$periodo)
                        ->where('materia',$mat->materia)
                        ->where('grupo',$mat->grupo)
                        ->select('rfc')
                        ->get();
                    if(!is_null($rfc[0]->rfc)){
                        $doc=DB::table('personal')->select('apellidos_empleado','nombre_empleado')
                            ->where('rfc',$rfc[0]->rfc)->first();
        ...
        if($tipo == 'D' || $tipo == 'P' || $tipo == 'G' || $tipo == 'K' || $tipo == 'B' || $tipo == '1S') //con tira de materias
        {
            ...
            if ($tipo == 'D' || $tipo == 'P' || $tipo == 'G' || $tipo == 'B' || $tipo == 'K') {
                ...
                if ($CU == 0) {
                    $suma=DB::table('historia_alumno')->where('no_de_control',$control)
                        ->where('calificacion','!=',0)
                        ->where('calificacion','!=',60)
                        ->get()
                        ->sum('calificacion');
                    $total=DB::table('historia_alumno')->where('no_de_control',$control)
                        ->where('calificacion','!=',0)
                        ->where('calificacion','!=',60)
                        ->distinct()
                        ->count('materia');
        ...
        if($tipo == 'D' || $tipo == 'P' || $tipo == 'G'){
            ...
            $this->fpdf->MultiCell($w-100,$h,'LAS CALIFICACIONES QUE AMPARA EL PRESENTE DOCUMENTO SERAN VALIDAS, PREVIO COTEJO DE LAS ACTAS CORRESPONDIENTES',0,'J');
        }
