                                ...
                                <?php
                                    for($i=2;$i<=7;$i++){
                                        $dias=\Illuminate\Support\Facades\DB::table('horarios')->where('periodo',$periodo)
                                            ->where('rfc',$rfc)->where('materia',$horario->materia)->where('grupo',$horario->grupo)
                                            ->where('tipo_horario','D')->where('dia_semana',$i)
...
@endif
<form action="{{route('acad_altaapoyo')}}" method="post" role="form">
    @csrf
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">Asignación de actividad de apoyo administrativo</div>
                    <div class="card-body">
                        <h4 class="card-title">Catálogo de acciones</h4>
                            <div class="form-group">
                                <label for="apoyo">Seleccione la actividad a realizar del siguiente listado</label>
                                <select name="apoyo" id="apoyo" required class="form-control">
                                    <option value="" selected>--Seleccione--</option>
                                    @foreach($apoyos as $apoyo)
                                        <option value="{{$apoyo->actividad}}">{{$apoyo->descripcion_actividad}}</option>
                                    @endforeach
                                </select>
                            </div>
                    </div>
                    <div class="card-body">
                        <div class="form-group">
                            <label for="especificar">Especifique la actividad</label>
                            <input type="text" name="especificar" id="especificar" required class="form-control" maxlength="100"
